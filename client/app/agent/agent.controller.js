...
.controller('AgentCtrl', function ($routeParams, $scope, $location, $http, AuthAgent, Agent) {
  AuthAgent.isLoggedInAsync(function (loggedIn) {
    if(!loggedIn) {
      AuthAgent.logout();
      $location.path('/');
    }
   ...
   */
  // Customer Profile
  if($location.path().indexOf('/agent/profile/customer/') > -1) {
    $http.get('/api/agent/customers/'+$routeParams.id)
  ...
  if( $location.path().indexOf('notifications') > -1 ||
      $location.path().indexOf('agent/list/customer') > -1 ||
      $location.path().indexOf('agent/list/ledger') > -1 ||
      $location.path().indexOf('agent/list/terms') > -1

    ) {
    $http.get('/api/agent/customers/')
  ...
  // Agent Profile
  if($location.path().indexOf('/agent/profile/agent/') > -1) {
    $http.get('/api/agent/agents/'+$routeParams.id)
      .success( function (data, status, headers, config) {
  ...
  // Agent List
  if($location.path() === '/agent/list/agent') {
    $http.get('/api/agent/agents/')
      .success( function (data, status, headers, config) {
        $scope.agents = data;
        console.log(data, status, headers, config);
      })
  ...
  // Committee Profile
  if($location.path().indexOf('/agent/profile/committee/') > -1) {
    $http.get('/api/agent/committees/'+$routeParams.committeeId)
      .success( function (data) {
  ...
  // Committee List
  if($location.path() === '/agent/list/committee') {
    $http.get('/api/agent/committees/')
      .success( function (data, status, headers, config) {
  ...
  // Committee Create
  if($location.path().indexOf('/agent/create/committee') > -1) {
    $http.get('/api/agent/customers/')
      .success( function (data, status, headers, config) {
        $scope.customers = data;
  ...
  $scope.updateCommittee = function(form) {
    ...
    if(form.$valid) {
      ...
      $http.put('/api/agent/committees/'+$scope.currentCommittee._id, newCommittee)
        .success( function (data, status, headers, config){
          $location.path('agent/list/committee');
          console.log(data, status, headers, config);
        })
      .error( function (data, status, headers, config){
        console.log(data, status, headers, config);
  ...
  $scope.deleteCommittee = function(committeeName) {
    ...
    $http.delete('/api/agent/committees/'+$routeParams.id)
      .success( function (data, status, headers, config){
        $location.path('agent/list/committee/');
        console.log(data, status, headers, config);
      })
  ...
  $scope.createCommittee = function(form) {
    ...
    if(form.$valid) {
      ...
      $http.post('/api/agent/committees', newCommittee)
        .success( function (data, status, headers, config){
          $location.path('agent/list/committee');
          console.log(data, status, headers, config);
        })
  ...
  $scope.addCommitteeLog = function(form) {
    ...
    if(form.$valid) {
      ...
      $http.put('/api/agent/committees/'+$scope.currentCommittee._id, $scope.currentCommittee)
        .success(function(data) {
          console.log(data);
          $location.path('/agent/profile/committee/'+$scope.currentCommittee._id);
        })
      .error(function(data) {
        console.log(data);
      });
    ...
    if(form.$valid) {
      ...
      for(var i = 0; i < updatedCustomer.terms.length; i++) {
        if(updatedCustomer.terms[i].title !== newTerm.title) {
          $location.path('agent/profile/customer/'+$scope.currentCustomer._id+'/terms');
          return;
        }
      }
      updatedCustomer.terms.push(newTerm);
      console.log(updatedCustomer);
      $http.put('/api/agent/customers/'+$routeParams.id, updatedCustomer)
        .success( function (data, status, headers, config){
          $location.path('agent/profile/customer/'+$scope.currentCustomer._id+'/terms');
          console.log(data, status, headers, config);
        })
  ...
  $scope.deleteTerm = function(termName) {
    ...
    for(var i = 0; i < $scope.currentCustomer.terms.length; i++) {
      if($scope.currentCustomer.terms[i].title === termName) {
        ...
        $http.put('/api/agent/customers/'+$routeParams.id, $scope.currentCustomer)
          .success( function (data, status, headers, config){
            $location.path('agent/profile/customer/'+$scope.currentCustomer._id+'/terms');
            console.log(data, status, headers, config);
          })
      ...
      for(var i = 0; i < $scope.currentCustomer.terms.length; i++) {
        if($scope.currentCustomer.terms[i].title === $scope.currentTerm.title) {
          ...
          $http.put('/api/agent/customers/'+$routeParams.id, $scope.currentCustomer)
            .success( function (data, status, headers, config) {
              console.log(data, status, headers, config);
              $location.path('agent/profile/customer/'+$scope.currentCustomer._id+'/terms/'+$scope.currentTerm.title+'/logs');
            })
          .error( function (data, status, headers, config){
            console.log(data, status, headers, config);
          });
  ...
  $scope.deleteCustomer = function(data) {
    $http.delete('/api/agent/customers/'+data._id) 
      .success(function (data, status, headers, config) {
        console.log(data, status, headers, config);
        $location.path('agent/list/customer/');
      })
    .error(function (data, status, headers, config) {
      console.log(data, status, headers, config);
    });
  ...
  $scope.createCustomer = function(form) {
    ...
    if(form.$valid) {
      $http.post('/api/agent/customers', $scope.createThisCustomer)
        .success( function (data, status, headers, config){
          $location.path('agent/list/terms/');
          console.log(data, status, headers, config);
        })
      .error( function (data, status, headers, config){
        console.log(data, status, headers, config);
  ...
  $scope._updateCustomer = function(customer) {
    $http.put('/api/agent/customers/'+customer._id, customer)
      .success( function (data, status, headers, config){
        $location.path('agent/profile/customer/'+customer._id);
        console.log(data, status, headers, config);
      })
    .error( function (data, status, headers, config){
      console.log(data, status, headers, config);
  ...
  $scope.updateCustomer = function(form) {
    $scope.submitted = true;
    if(form.$valid) {
      $http.put('/api/agent/customers/'+$routeParams.id, $scope.updateThisCustomer)
        .success( function (data, status, headers, config){
          $location.path('agent/list/customer/');
          console.log(data, status, headers, config);
        })
  ...
  $scope.createAgent = function(form) {
    $scope.submitted = true;
    if(form.$valid) {
      $http.post('/api/agent/agents', $scope.createThisAgent)
        .success( function (data, status, headers, config){
          $location.path('agent/list/agent/');
          console.log(data, status, headers, config);
        })
      .error( function (data, status, headers, config){
        console.log(data, status, headers, config);
  ...
  $scope.update = function(form) {
    $scope.submitted = true;
    if(form.$valid) {
      AuthAgent.update($scope.agent.updated)
        .then( function() {
          $scope.message = 'Details updated!';
          $location.path('/agent/profile');
        })
      .catch( function(err) {
        err = err.data;
        $scope.errors = {};
  ...
  $scope.computeInstallmentCount = function(startDate, duration, installments) {
    ...
    } else if(installments.duration.paramter === "years") {
